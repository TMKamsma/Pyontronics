...
def objective_esn(trial):
    ...
    mse = 0
    if (targets_bpntest[375, 0] - predictions[375, 0]) ** 2 < 100:
...
def objective_bpn(trial):
    ...
    # Initialize BPN (BandPassNetwork)
    bpn = BandPassNetwork(
        input_dim=1,
        reservoir_size=reservoir_size,
        output_dim=1,
...
study_name = "ESN optimization HWTS"
...
study.optimize(objective_esn, n_trials=800, n_jobs=-1)

study_name = "BPN optimization HWTS"
storage = optuna.storages.RDBStorage(
    "sqlite:///optuna_esn.db", engine_kwargs={"connect_args": {"timeout": 20.0}}
)
try:
...
study.optimize(objective_bpn, n_trials=800, n_jobs=-1)