...
class BandPassNetwork:
    def __init__(
        ...
        self.time_scale_avg = time_scale_avg
        ...
        self.timescale_array = np.ones(self.reservoir_size)
        ...
        self._initialize_timescales()

    def _check_parameters(self):
        """Checks whether the chosen parameters are valid."""
        if self.leaking_rate * (self.step_size / self.time_scale_avg) > 1:
            raise ValueError("leaking_rate * (step_size / time_scale_avg) must be ≤ 1.")
    ...
    def _initialize_all_weights(self):
        ...
        max_eig_M = np.max(
            np.abs(
                np.linalg.eigvals(
                    (self.step_size / self.time_scale_avg) * np.absolute(self.W_res)
                    - (1 - self.leaking_rate * (self.step_size / self.time_scale_avg))
    ...
    def _initialize_timescales(self):
        self.timescale_array = np.random.normal(loc=self.time_scale_avg, scale=self.time_scale_std, size=self.reservoir_size);
        self.timescale_array = (self.timescale_array).clip(min=self.time_scale_avg/5)
    ...
    def physical_length(self) -> float:
        ...
        tau = self.time_scale_avg / self.leaking_rate
        D = 1e-9  # Diffusion constant in m²/s
        return np.sqrt(12 * D * tau) * 1e6  # Convert to micrometers

class PulseBandPassNetwork(BandPassNetwork):
    """
